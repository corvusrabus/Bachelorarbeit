...
Although the tensor algebra is a very nice algebra it has the problem that it's not commutative in general. We can fix this by
...
To get a better understanding of this structure we will now present some examples.
 ...
 \begin{Lemma}
  Let $\K$ be an algebraically closed field and $(\Lambda V,d)$ a minimal Sullivan algebra over $\K$ with finite $V^i$ for all $i \geq 0$. \newline
 ...
 Since boundaries get mapped to boundaries $\bar{\varphi}(v^k)$ is a coboundary. And $(\K[\alpha],0)$ is zero in odd degrees thus
